...
**Eccezione per i crawler.** `auto=format` dipende dall'header `Accept`, che gli scraper social non mandano in modo affidabile. Gli URL destinati ai crawler — Open Graph, Twitter card, sitemap — fissano il formato con `fm=jpg` e dimensioni esplicite (`w=1200&h=630&fit=crop`) e non passano dal loader di `next/image`.
